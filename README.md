...
- `app/Http/Middleware/SecurityHeaders.php` adds a Content Security Policy and the other
  ...
  headers, and why each is there, are in [docs/deployment.md](docs/deployment.md).
...
| **Target environment**  | We develop against `php artisan serve`. The brief specifies Apache via XAMPP on Windows, assessed in Chrome. Somebody needs to run the app that way and confirm it behaves, well before submission. The Apache setup steps are written up in [docs/deployment.md](docs/deployment.md#1-running-it-on-apache-xampp-on-windows), but have not been tried on Windows yet. |
...
| Privacy notice                | Written (§9): `/privacy`, linked from every page's footer and the registration form. It covers what is stored and why, the cookies, and how to delete an account; see [Privacy notice](#privacy-notice--privacy). |
| Deployment write-up           | Written (§10) in [docs/deployment.md](docs/deployment.md#4-secure-deployment-design-10): the network layout, firewall rules, administration access, secrets, logs, backups and separate environments, ready to adapt for the report. |

#### Optional

Built: the read-only JSON search endpoint from §5 of the proposal,
`GET /api/recipes`; see [JSON search API](#json-search-api--apirecipes). The listing page
