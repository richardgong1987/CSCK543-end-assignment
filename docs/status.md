...
| **Target environment**  | We develop against `php artisan serve`. The brief specifies Apache via XAMPP on Windows, assessed in Chrome. Somebody needs to run the app that way and confirm it behaves, well before submission. The steps, including replacing the PHP 8.2 that XAMPP bundles with PHP 8.4, are in [xampp.md](xampp.md), but have not been tried on Windows yet. |
