...
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Accessibility**       | Automated checks, a VoiceOver pass and a scripted keyboard-only pass done; the keyboard-only check by a person is still to do. **VoiceOver and keyboard-only passes on 19 September 2026**: five VoiceOver scenarios (headings, form errors, the favourite confirmation, the star rating) and fifteen keyboard steps all behaved as intended, with the transcripts and screenshots in [accessibility.md](accessibility.md); one usability finding, 35 Tab presses to reach the first search result. Every page has exactly one `<h1>`, a `<main>` landmark, labelled controls, alt text on every image and a sensible heading order; pages with navigation also have a skip link, and the filter groups use `<fieldset>`/`<legend>` inside a `role="search"` form. Form errors, from JavaScript and from the server, are tied to their field with `aria-describedby` and `aria-invalid`, and a failed submit moves focus to the first invalid field. **Checked on 15 September 2026** in Chrome with axe-core 4.13 (WCAG 2.0, 2.1 and 2.2 A/AA plus best practices), in light and dark mode: no violations on the home page, the recipe listing, a recipe page (as a guest and logged in, with the rating form), login, registration (including the client-side and server-side error states), the account page, the account settings page (including a server error on the delete form), the forgot password and reset password pages (including their status message and a refused token), and the privacy notice. A scripted keyboard pass over the same pages reached every control with Tab and found a visible focus change on each. Colour contrast was measured for the whole palette and fixed where it fell short: accent-red text darkened from `#f53003` to `#d32903` (3.9:1 → 5.1:1), the JavaScript error text given a dark-mode colour, input and select borders raised to 3:1 (`#91918f` light, `#676763` dark, for WCAG 1.4.11), and placeholders to `#767570`. The audit was a one-off script, not yet in the repository; it belongs with the end-to-end tests. Still to do: the keyboard-only check by a person (the checklist in accessibility.md), and NVDA on Windows. |
| **Responsive layout**   | **Checked on 15 September 2026** in Chrome's device emulation at 360, 390, 768 and 1280 px wide, on every page as a guest and signed in (48 page loads), and **on 19 September 2026 in real mobile Chrome** on an Android 16 phone emulator (Pixel 9, 411 px) and a tablet emulator (800 px portrait, 1280 px landscape), over Wi-Fi to the XAMPP site: 27 more page loads, no sideways scrolling anywhere, with screenshots in [responsive.md](responsive.md). The login and registration forms had a fixed width wider than a phone, and now shrink to fit. Still to do: the check on a real iPhone, iPad, Android phone and tablet (responsive.md, section 5). |
...
| Work                          | Where it stands                                                                                                     |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| Unit and feature tests        | 198 tests covering the schema, authentication, password reset, search, sorting, favourites, ratings, the account page, account settings, the JSON API, security headers, query budgets and the other pages. Extend these as features land.          |
...
| Performance testing           | **All 18 acceptance criteria met on 19 September 2026** ([performance.md](performance.md)), measured on the XAMPP stack with Lighthouse 13.4 (mobile) and curl: performance 100 and accessibility, best practices and SEO 100 on home, listing, a recipe and login; largest paint 1.4–1.8 s; layout shift 0 and blocking time 0 ms everywhere; page weight 76–266 KiB; server response p95 20–51 ms, and 49 ms for a search. The first measurement met 14: the three fixes were text compression in `public/.htaccess`, fonts as WOFF2 only, and WebP copies of each recipe photo in three widths served with `srcset`. `QueryBudgetTest` pins each page's query count, and `Model::preventLazyLoading()` turns any N+1 query into an error; none was found. Caching headers for a real server are in [deployment.md](deployment.md#caching-the-built-assets). Awaiting the group's sign-off of the criteria. |
| Web sustainability            | **All 12 criteria met on 19 September 2026** ([sustainability.md](sustainability.md)), based on the W3C Web Sustainability Guidelines: every page rated A+ by the Sustainable Web Design model (0.011–0.029 g CO2e per view, measured in Chrome at phone size); a repeat visit sends one request, the page, because `public/.htaccess` now gives built files a year's cache lifetime and photos a week; photos below the first screen load only on scrolling; no third-party requests; dark mode follows the device. A CDN, green hosting and Brotli are recommended for production, not done for the XAMPP submission. Awaiting the group's sign-off of the criteria. |
| Load and stress testing       | **Run on 19 September 2026** with k6 against the XAMPP copy: the load test (20 users) handled 550 requests a second with 0 % errors and p95 62.7 ms; the stress test stayed clean up to about 124 concurrent users, at about 530 requests a second, then refused requests with 503 once PHP-FPM's 5 processes and their queue were full, and recovered as soon as the load fell. The tests also found, and we fixed, one database TCP connection per request exhausting network ports. Numbers, screenshots and recommendations in [load-testing.md](load-testing.md). |
